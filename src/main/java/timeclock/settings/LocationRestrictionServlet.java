package timeclock.settings; // Your package
...
public class LocationRestrictionServlet extends HttpServlet {
    ...
    private static final String WIZARD_RETURN_STEP_SETTINGS = "settings_setup"; // Step on settings.jsp to return to
    ...
    private void loadAndForward(HttpServletRequest request, HttpServletResponse response, Integer tenantId, 
        ...
        if(submittedInWizardMode && wizardStepToReturnToOnSettingsPage == null){
        ...
        // --- REDIRECT LOGIC FOR WIZARD MODE ---
        if (pageIsActuallyInWizardMode && wizardStepToReturnToOnSettingsPage != null) {
            if (saveSuccessMsg != null && !saveSuccessMsg.isEmpty()) {
                String redirectUrl = request.getContextPath() + "/settings.jsp?setup_wizard=true&step=" +
                                     ...
                                     "&restrictionConfigured=location"; // Indicate location was done
                logger.info("[LocationRestrictionServlet] Successful save in wizard. Redirecting to: " + redirectUrl);
                response.sendRedirect(redirectUrl);
                return;
            }
            // If there was an error during a POST save action while in wizard mode
            if (saveErrorMsg != null && !saveErrorMsg.isEmpty() && request.getMethod().equalsIgnoreCase("POST") ) {
                ...
                logger.info("[LocationRestrictionServlet] Error during save in wizard. Redirecting to: " + redirectUrl);
                response.sendRedirect(redirectUrl);
                return;
            }
        }

        // --- Regular Forward to configureLocationRestrictions.jsp ---
        ...
        if (tenantId != null && tenantId > 0) {
            ...
            try (Connection conn = DatabaseConnection.getConnection();
                ...
                logger.info("Fetched " + locations.size() + " geofence locations for TenantID: " + tenantId);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        logger.info("LocationRestrictionServlet doGet called.");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.info("LocationRestrictionServlet doPost called.");
        Integer tenantId = getTenantId(request);
        ...
        if (tenantId == null || tenantId <= 0) {
            ...
            // For POST, if session is bad, a general redirect to login is better than forwarding
            response.sendRedirect(request.getContextPath() + "/login.jsp?error=" + encodeURL(errorMessage));
            return;
        }
        if (!isAdmin(request)) {
            errorMessage = "Access Denied.";
            // If not admin on POST, redirect to settings page with error, not the config page.
            response.sendRedirect(request.getContextPath() + "/settings.jsp?error=" + encodeURL(errorMessage));
            return;
        }
        ...
        try {
            ...
            if ("addLocation".equals(action) || "editLocation".equals(action)) {
                ...
                if ("addLocation".equals(action)) {
                    ...
                    try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                        ...
                        if (rowsAffected > 0) successMessage = "Location '" + name.trim() + "' updated successfully.";
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    ...
                    int rowsAffected = pstmt.executeUpdate();
                    if (rowsAffected > 0) successMessage = "Location deleted successfully.";
                    else errorMessage = "Location not found or already deleted.";
                }
            } else if ("updateLocationStatus".equals(action)) {
                int locationID = Integer.parseInt(request.getParameter("locationID"));
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                    pstmt.setBoolean(1, isEnabled); pstmt.setInt(2, locationID); pstmt.setInt(3, tenantId);
                    int rowsAffected = pstmt.executeUpdate();
                    if (rowsAffected > 0) successMessage = "Location status updated.";
                    else errorMessage = "Failed to update location status.";
                }
            } else {
            ...
            logger.log(Level.WARNING, "Invalid number format for location data, TenantID: " + tenantId, e);
            errorMessage = "Invalid input: Ensure numeric fields are correct.";
            if (conn != null) try { conn.rollback(); } catch (SQLException ex) { logger.log(Level.SEVERE, "Rollback failed.", ex); }
        } catch (IllegalArgumentException e) {
            logger.log(Level.WARNING, "Invalid argument for location data, TenantID: " + tenantId + ". Error: " + e.getMessage());
            ...
            logger.log(Level.SEVERE, "Database error processing location restriction for TenantID: " + tenantId, e);
