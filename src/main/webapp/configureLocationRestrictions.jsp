...
<head>
    ...
    <style>
        ...
        .location-table td .action-icon { margin-right: 10px; color: #007bff; cursor:pointer; font-size: 1.1em; } 
        .location-table td .action-icon.delete { color: #dc3545; } 
        .location-table .switch { margin-left:0; }
        #locationModal .modal-content { max-width: 600px; }
        .required-asterisk { color: red; margin-left: 2px;} 
        #locationMap { height: 280px; margin-bottom: 10px; border: 1px solid #ccc; border-radius: 4px; }
        .map-instructions { font-size: 0.85em; text-align: center; margin-top: -5px; margin-bottom: 15px; padding: 8px 10px; background-color: #f0f5fa; border: 1px solid #d6e0f0; border-radius: 4px; color: #334e68; } 
        .lat-lon-group { display: flex; gap: 10px; } .lat-lon-group .form-item { flex: 1; } 
        #confirmModal .modal-content p { padding: 20px 10px; text-align: center; line-height: 1.6; }
        
        /* MODIFIED: Added margin-bottom to the action-buttons container */
...
<body class="reports-page">
    ...
    <div class="parent-container config-container">
        ...
        <% if (errorMessageFromPost != null) { %><div class="page-message error-message"><i class="fas fa-exclamation-triangle"></i> <%= escapeHtml(errorMessageFromPost) %></div><% } %>
